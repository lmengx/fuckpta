...
- **直接安装**：下载 [dist.zip](https://gitee.com/lmx12330/fuckpta/blob/main/dist.zip) 下载最新版本的扩展包，然后在浏览器扩展管理页面加载已解压的扩展。

### 对于开发者

- **使用 npm 进行包管理**：
  1. 克隆仓库：`git clone https://github.com/lmengx/fuckpta.git`
  2. 安装依赖：`cnpm install`
  3. 构建项目：`cnpm run build`
...
1. **安装扩展**：
   - 克隆仓库
   - 运行 `cnpm install` 安装依赖
   - 运行 `cnpm run build` 构建项目
